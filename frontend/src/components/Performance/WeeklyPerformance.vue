      ...
      <div v-else class="chart-container">
        <div class="chart">
          ...
          >
            ...
            <div
              v-if="day.count > 0 && day.count === getMaxCount()"
              class="star"
...
const weeklyData = computed(() => {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const daySprints = sprintStore.sprints.filter((sprint) => sprint.started_at.startsWith(dateStr))

    const totalSeconds = daySprints.reduce((total, sprint) => {
      return total + parseInt(sprint.duration)
    }, 0)

    const totalMinutes = Math.round(totalSeconds / 60000)
